...
export const HorizontalSteps = React.forwardRef<HTMLButtonElement, HorizontalStepsProps>(({color = 'primary', steps = [], defaultStep = 0, onStepChange, currentStep: currentStepProp, hideProgressBars = false, stepClassName, className, ...props}, ref) => {
	...
	const colors = React.useMemo(() => {
		...
		const colorsVars = ['[--active-fg-color:hsl(var(--step-fg-color))]', '[--active-border-color:hsl(var(--step-color))]', '[--active-color:hsl(var(--step-color))]', '[--complete-background-color:hsl(var(--step-color))]', '[--complete-border-color:hsl(var(--step-color))]', '[--inactive-border-color:hsl(var(--heroui-default-300))]', '[--inactive-color:hsl(var(--heroui-default-300))]'];
				...
				{steps?.map((step, stepIdx) => {
					...
					return (
						<li key={stepIdx} className='relative flex items-center w-full'>
							<button key={stepIdx} ref={ref} aria-current={status === 'active' ? 'step' : undefined} className={cn('group flex w-full cursor-pointer flex-col items-center justify-center gap-y-2 rounded-large py-2.5', stepClassName)} onClick={() => setCurrentStep(stepIdx)} {...props}>
								<div className='relative flex items-center h-ful'>
									<LazyMotion features={domAnimation}>
										<m.div animate={status} className='relative'>
											<m.div
												className={cn('relative flex h-[28px] w-[28px] items-center justify-center rounded-full border-medium text-large font-semibold text-default-foreground', {
												...
												<div className='flex items-center justify-center'>{status === 'complete' ? <CheckIcon className='h-6 w-6 text-(--active-fg-color)' /> : <span>{stepIdx + 1}</span>}</div>
											</m.div>
										</m.div>
									</LazyMotion>
									{stepIdx < steps.length - 1 && !hideProgressBars && (
										<div
											aria-hidden='true'
											className={cn('pointer-events-none absolute left-6 top-1/2 flex w-12 -translate-y-1/2 translate-x-1/2 items-center sm:w-8')}
											style={{
												// @ts-ignore
												'--idx': stepIdx
											}}>
											<div
												className={cn('relative h-0.5 w-full bg-default-200 transition-colors duration-300', 'after:absolute after:block after:h-full after:w-0 after:bg-(--active-border-color) after:transition-[width] after:duration-300 after:content-["""]', {
