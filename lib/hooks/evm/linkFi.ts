...
// 	const {getAccountData} = useNeuraFiData({token});
// 	const {addTransaction} = useTransactionManager({onComplete: () => getAccountData()});
...
// 	const _getAccountData = useCallback(async () => {
...
// 	//useMount(() => _getAccountData()); //挂载后获取一次
// 	useEffect(() => {
// 		account && _getAccountData();
// 	}, [account]);

// 	const getAccountData = withError({title: 'getAccountData', onStart: () => setIsLoading(true), onComplete: () => setIsLoading(false)})(_getAccountData);
// 	return {isLoading, userData, getAccountData};
// };
